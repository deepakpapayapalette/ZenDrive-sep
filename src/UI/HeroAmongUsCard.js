...
import LifeSaverShare from "../components/website/Modals/LifeSaverShare";
...
const SuperheroThankYouCard = ({ user }) => {
  ...
  return (
    <>
      <div className="w-full bg-white rounded-lg theme-shadow overflow-hidden relative">
        ...
        <div className="bg-[#eaf0f2] p-4 pt-5 mt-2">
          <div className="flex items-center justify-between mb-4">
            ...
            {/* Share Button */}
            <button
              className=" border-primary border-2 rounded-md px-3 py-1 text-sm text-primary hover:bg-gray-100 flex items-center"
      ...
      {isShareModalOpen && (
        <LifeSaverShare
          itemId={user?.id || user?.phone || 'default'}
