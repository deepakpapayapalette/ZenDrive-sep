...
const ColorMaster = () => {
    ...
    useEffect(() => {
        fetchData(['color_type'],);
    }, []);

    ///========== handle form submit ============\\
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!colorMaster.trim() && colorMaster.length === 0) {
            toast.error("Please enter a valid Color Master");
            return;
        }
        setIsLoading(true);
        ...
        try {
            const payload = {
                ...
                "lookup_value": colorMaster,
                "lookup_type": "color_type",
                "parent_lookup_type": "",
                "parent_lookup_id": null
            }
            const res = await __postApiData('/api/v1/admin/SaveLookup', payload);
            if (res.response && res.response.response_code === "200") {
                toast.success(editId ? "Color Master updated successfully" : "Color Master added successfully");
                setColorMaster("");
                setEditId(null);
                setIsLoading(false);
                fetchData(['color_type']);
    ...
    const handleEdit = (row) => {
        setColorMaster(row?.lookup_value);
        setEditId(row?._id);
    };
        ...
        try {
            ...
            if (result.isConfirmed) {
                ...
                if (res?.response?.response_code === "200") {
                    toast.success("Color Master deleted successfully");
                    fetchData(['color_type']);
                }
            }
        } catch (error) {
    ...
    return (
        <div className="p-4 bg-white">
            <SectionHeader
                title="Enter Details for Color Master"
                description="Add or update the required details for the color master to keep records accurate and complete."
            />
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-8 shadow-lg  rounded-md p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormInput
                        label="Color master"
                        name="colorMaster"
                        placeholder="Enter Color master"
                        value={colorMaster}
                        onChange={(e) => setColorMaster(e.target.value)}
                    />

                </div>

                <div className="mt-4">
                    <FormButton disabled={isLoading}>{isLoading ? editId ? " Updating..." : "Adding..." : editId ? "Update Color" : "Add Color"}</FormButton>
                </div>
            </form>
